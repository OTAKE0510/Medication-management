    ...
    function addScheduleRow() {
        ...
        row.innerHTML = `<input type="text" name="timing-text" placeholder="例：朝、昼、夕、晩、食前、食後など自由記入"><input type="number" name="dosage-amount" min="1" value="1" style="width:60px"> 錠 <input type="time" name="notify-time" min="00:00" max="23:59"> <button type="button" class="remove-schedule-btn">削除</button>`;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    form.addEventListener("submit", (e) => {
        ...
        scheduleRows.forEach(row => {
            ...
            // 必須項目が入力されているかチェック
            if (timingInput.value && dosageInput.value && notifyTimeInput.value) {
                schedule.push({
                    timing: timingInput.value,
                    dosage: Number(dosageInput.value),
                    notifyTime: notifyTimeInput.value
                });
            } else if (form['interval-type'].value !== 'tonpuku') {
                // 頓服でなければスケジュールは必須
